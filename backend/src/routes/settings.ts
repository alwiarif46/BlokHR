 ...
 *   POST /api/members                 — create a new member (admin-only) — proxies to directory when available
...
export function createSettingsRouter(
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (directory) {
        const result = await directory.createMember({
        ...
        if (!result.success) {
          throw new AppError(result.error ?? 'Failed to create member', result.status ?? 400);
        }
        if (broadcaster) {
