...
describe('school-library P12-04 role guard', () => {
  let app: Express;
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of LIBRARY_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/library${samplePath(policy.pattern)}`;
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/library/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = LIBRARY_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/library${samplePath(first.pattern)}`
      : '/api/library/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
