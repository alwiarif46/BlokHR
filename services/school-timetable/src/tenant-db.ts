...
export function createTimetableTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolTimetableSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-timetable.db',
    env: opts.env,
    create: (p) => SchoolTimetableSqlite.create(p),
    migrate: (db) => runSchoolTimetableMigrations(db, opts.migrationsDir),
  });
}
