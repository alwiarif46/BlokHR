...
describe('directory P12-04 role guard', () => {
  let app: Express;
  ...
  beforeEach(async () => {
    const created = await createDirectoryApp({
      ...
      seatChecker: { checkSeats: async () => ({ allowed: true, seatLimit: 100, activeSeats: 0 }) }, auth: { createCredentials: async () => ({ success: true }) },
    });
    app = created.app;
    db = created.db;
  });
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of DIRECTORY_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/directory${samplePath(policy.pattern)}`;
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/directory/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = DIRECTORY_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/directory${samplePath(first.pattern)}`
      : '/api/directory/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
