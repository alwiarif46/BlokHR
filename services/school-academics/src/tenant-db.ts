...
export function createAcademicsTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolAcademicsSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-academics.db',
    env: opts.env,
    create: (p) => SchoolAcademicsSqlite.create(p),
    migrate: (db) => runSchoolAcademicsMigrations(db, opts.migrationsDir),
  });
}
