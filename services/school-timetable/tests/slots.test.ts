...
describe('school-timetable slots', () => {
  ...
  beforeEach(async () => {
    ...
    const aMath = await request(app).post('/api/timetable/t1/allocations').send({
      section_id: sectionA,
    ...
    const bMath = await request(app).post('/api/timetable/t1/allocations').send({
      section_id: sectionB,
      subject_id: math.body.id,
      teacher_member_id: 'teacher-math',
      periods_per_week: 5,
    ...
    const aSci = await request(app).post('/api/timetable/t1/allocations').send({
      section_id: sectionA,
  ...
  it('replaces and reads section slots with subject/teacher join', async () => {
    ...
    const got = await request(app).get(`/api/timetable/t1/sections/${sectionA}/slots`);
  ...
  it('returns teacher week view', async () => {
    ...
    const week = await request(app).get('/api/timetable/t1/teachers/teacher-math/slots');
    ...
    const sci = await request(app).get('/api/timetable/t1/teachers/teacher-sci/slots');
  ...
  it('isolates tenants', async () => {
    ...
    const other = await request(app).get(`/api/timetable/tenant-b/sections/${sectionA}/slots`);
