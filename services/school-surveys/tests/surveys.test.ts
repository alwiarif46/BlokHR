...
describe('school-surveys', () => {
  ...
  it('creates, lists, publishes, and closes a survey', async () => {
    const created = await request(app).post('/api/surveys/t1').send({
      title: 'Parent pulse',
    ...
    const list = await request(app).get('/api/surveys/t1');
  ...
  it('rejects invalid question type', async () => {
    const res = await request(app).post('/api/surveys/t1').send({
      title: 'Bad',
  ...
  it('enforces tenant isolation', async () => {
    const a = await request(app).post('/api/surveys/tA').send({ title: 'Secret' });
    const id = a.body.survey.id as string;
    expect((await request(app).get(`/api/surveys/tB/${id}`)).status).toBe(404);
    expect((await request(app).get('/api/surveys/tB')).body.surveys).toEqual([]);
  });
  ...
  it('filters guardian pending by X-Blok-Students and ignores spoofed guardian_ref', async () => {
    ...
    await request(app).post(`/api/surveys/t1/${id}/publish`);
