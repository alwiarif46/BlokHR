...
// sql.js does not always resolve DefinitelyTyped when linked from the gateway package
...
/** Lightweight sql.js wrapper owned by the kiosk service. */
export class KioskSqlite implements KioskDb {
  ...
  static async create(dbPath: string): Promise<KioskSqlite> {
    const engine = new KioskSqlite(dbPath);
...
export async function runKioskMigrations(db: KioskDb, migrationsDir: string): Promise<void> {
  await db.run(`
