...
describe('school-identity', () => {
  ...
  it('creates and lists academic sessions', async () => {
    const created = await request(app).post('/api/identity/t1/sessions').send({
      label: '2025-26',
    ...
    const list = await request(app).get('/api/identity/t1/sessions');
  ...
  it('enforces is_current exclusivity per tenant', async () => {
    await request(app).post('/api/identity/t1/sessions').send({
      label: 'Old',
  ...
  it('rejects invalid dates with 400', async () => {
    const badIso = await request(app).post('/api/identity/t1/sessions').send({
      label: 'X',
    ...
    const order = await request(app).post('/api/identity/t1/sessions').send({
      label: 'X',
  ...
  it('isolates tenants', async () => {
    await request(app).post('/api/identity/tenant-a/sessions').send({
      label: 'A-only',
      starts_on: '2025-04-01',
      ends_on: '2026-03-31',
    });
    const b = await request(app).get('/api/identity/tenant-b/sessions');
    expect(b.status).toBe(200);
    expect(b.body.sessions).toHaveLength(0);

    const a = await request(app).get('/api/identity/tenant-a/sessions');
