...
import { registerModule, renderModuleInto } from '../../shared/router.js';
...
export function switchDashTab(tabKey, container) {
  _activeTab = tabKey;
  const tabContainer = container || _dashRoot || document;
  ...
  const mod = moduleMap[tabKey];
  if (mod) {
    renderModuleInto(mod, content);
    return;
  }

  content.innerHTML =
    '<div class="mod-coming"><div class="mod-coming-title df">Unknown tab</div></div>';
}
...
async function dashLoadData(container) {
  ...
  const [attendance, leaveBalances] = await Promise.all([
    ...
    session.email
      ? api.get('/api/leaves/balances?email=' + encodeURIComponent(session.email))
  ...
  /* Only refresh the home tab — don't wipe Leaves/Profile/etc. mid-view */
