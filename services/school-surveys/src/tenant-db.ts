...
export function createSurveysTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolSurveysSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-surveys.db',
    env: opts.env,
    create: (p) => SchoolSurveysSqlite.create(p),
    migrate: (db) => runSchoolSurveysMigrations(db, opts.migrationsDir),
  });
}
