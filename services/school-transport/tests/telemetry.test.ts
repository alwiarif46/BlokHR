...
describe('school-transport telemetry (P7-03)', () => {
  ...
  beforeEach(async () => {
    ...
    const stop = await request(app)
      .post(`/api/transport/t1/routes/${routeId}/stops`)
      .send({
        label: 'East Stop',
        lat: 0,
        lng: 15 / 111.195,
  ...
  it('ping validation + batch cap', async () => {
    const bad = await request(app).post('/api/transport/t1/pings').send([
    ...
    const ok = await request(app).post('/api/transport/t1/pings').send([
      {
        vehicle_id: vehicleId,
        lat: 0,
        lng: 0,
  ...
  it('prune + last-known staleness', async () => {
    await request(app).post('/api/transport/t1/pings').send([
      {
        vehicle_id: vehicleId,
        lat: 1,
        lng: 1,
    ...
    const pruned = await request(app).post('/api/transport/t1/pings/prune');
  ...
  it('ETA fresh vs stale; delay once-per-day; tenant isolation', async () => {
    ...
    const d1 = await request(app)
      .post(`/api/transport/t1/routes/${routeId}/check-delay`)
      .send({ leg: 'pickup' });
