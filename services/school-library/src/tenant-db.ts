...
export function createLibraryTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolLibrarySqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-library.db',
    env: opts.env,
    create: (p) => SchoolLibrarySqlite.create(p),
    migrate: (db) => runSchoolLibraryMigrations(db, opts.migrationsDir),
  });
}
