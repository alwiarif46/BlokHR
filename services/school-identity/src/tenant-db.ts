...
export function createIdentityTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolIdentitySqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-identity.db',
    env: opts.env,
    create: (p) => SchoolIdentitySqlite.create(p),
    migrate: (db) => runSchoolIdentityMigrations(db, opts.migrationsDir),
  });
}
