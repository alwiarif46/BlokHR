...
describe('school-timetable P12-04 role guard', () => {
  let app: Express;
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of TIMETABLE_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/timetable${samplePath(policy.pattern)}`;
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/timetable/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = TIMETABLE_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/timetable${samplePath(first.pattern)}`
      : '/api/timetable/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
  ...
  it('teacher without X-Blok-Member → no_member_binding on slots', async () => {
    const res = await request(app)
      .get('/api/timetable/t1/teachers/member-teacher-1/slots')
      .set(staff('teacher', null));
    expect(res.status).toBe(403);
    expect(res.body.error).toBe('no_member_binding');
  });
});
