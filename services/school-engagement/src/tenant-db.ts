...
export function createEngagementTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolEngagementSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-engagement.db',
    env: opts.env,
    create: (p) => SchoolEngagementSqlite.create(p),
    migrate: (db) => runSchoolEngagementMigrations(db, opts.migrationsDir),
  });
}
