...
describe('school-academics curriculum (P3-02)', () => {
  ...
  async function createCourse(tenant = 't1') {
    const res = await request(app).post(`/api/academics/${tenant}/courses`).send({
  ...
  it('assembles course tree with units, topics, and outcome tags', async () => {
    ...
    const u1 = await request(app)
      .post(`/api/academics/t1/courses/${course.id}/units`)
    ...
    const topic = await request(app)
      .post(`/api/academics/t1/units/${u1.body.id}/topics`)
    ...
    const act = await request(app)
      .post(`/api/academics/t1/units/${u1.body.id}/outcomes`)
      .send({ outcome_id: outcomeId, field: 'activity', depth: 'reinforced' });
    expect(act.status).toBe(201);
    expect(act.body.depth).toBe('reinforced');

    const tree = await request(app).get(`/api/academics/t1/courses/${course.id}/tree`);
  ...
  it('reorders units', async () => {
    ...
    const reordered = await request(app)
      .put(`/api/academics/t1/courses/${course.id}/units/reorder`)
  ...
  it('assessment requires activity; depth values validated', async () => {
    ...
    const bad = await request(app)
      .post(`/api/academics/t1/units/${unit.body.id}/outcomes`)
  ...
  it('tenant isolation for courses', async () => {
    ...
    const list1 = await request(app).get('/api/academics/t1/courses');
    const list2 = await request(app).get('/api/academics/t2/courses');
    ...
    const steal = await request(app).get(`/api/academics/t2/courses/${c1.id}/tree`);
