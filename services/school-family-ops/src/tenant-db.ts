...
import { SchoolFamilyOpsSqlite, runSchoolFamilyOpsMigrations } from './db';

export function createFamilyOpsTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolFamilyOpsSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-family-ops.db',
    env: opts.env,
    create: (p) => SchoolFamilyOpsSqlite.create(p),
    migrate: (db) => runSchoolFamilyOpsMigrations(db, opts.migrationsDir),
  });
}
