...
export function createComplianceTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolComplianceSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-compliance.db',
    env: opts.env,
    create: (p) => SchoolComplianceSqlite.create(p),
    migrate: (db) => runSchoolComplianceMigrations(db, opts.migrationsDir),
  });
}
