...
describe('school-engagement messages (P5-02)', () => {
  ...
  it('seeds global templates; resolution chain tenant→lang→en', async () => {
    ...
    await request(app).post('/api/engagement/t1/templates').send({
      key: 'attendance_nudge',
      lang: 'hi',
      body: 'Tenant HI nudge for {{student_name}} on {{date}}.',
    });

    const hi = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's1',
    ...
    const frFallback = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's2',
  ...
  it('var validation; channel walk; digest queue', async () => {
    const missing = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      template_key: 'absence_alert',
      urgency: 'interrupt',
      vars: { student_name: 'A' },
    ...
    const sent = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's1',
      template_key: 'absence_alert',
      urgency: 'interrupt',
    ...
    const digest = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's1',
      template_key: 'attendance_nudge',
      urgency: 'digest',
  ...
  it('daily cap and quiet hours with absence exception', async () => {
    ...
    const quietNudge = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's9',
      template_key: 'attendance_nudge',
      urgency: 'interrupt',
    ...
    const quietAbsence = await request(app).post('/api/engagement/t1/messages').send({
      guardian_ref: 'g1',
      student_ref: 's9',
      template_key: 'absence_alert',
      urgency: 'interrupt',
  ...
  it('tenant isolation', async () => {
    ...
    const other = await request(app).get(
      '/api/engagement/t2/messages?student_ref=s1',
    );
