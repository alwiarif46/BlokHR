...
describe('school-attendance reason codes', () => {
  ...
  it('seeds default reason codes on first read', async () => {
    const res = await request(app).get('/api/attendance/t1/reason-codes');
    expect(res.status).toBe(200);
  ...
  it('creates and patches reason codes; bucket is immutable', async () => {
    await request(app).get('/api/attendance/t1/reason-codes');

    const created = await request(app).post('/api/attendance/t1/reason-codes').send({
      code: 'trip',
    ...
    const patched = await request(app)
      .patch(`/api/attendance/t1/reason-codes/${created.body.id}`)
  ...
  it('isolates tenants', async () => {
    await request(app).get('/api/attendance/tenant-a/reason-codes');
    await request(app).post('/api/attendance/tenant-a/reason-codes').send({
      code: 'AONLY',
      label: 'A only',
      bucket: 'authorised',
    });

    const b = await request(app).get('/api/attendance/tenant-b/reason-codes');
    ...
    const a = await request(app).get('/api/attendance/tenant-a/reason-codes');
