...
import { renderLeavePolicyManager } from '../leave_policies/leave_policy_manager.js';
...
const SECTIONS = [
  ...
  /* §6.2 */ { key: 'attendance', title: 'Attendance Rules', icon: '\u23F0', fields: [
    ...
    { id: 'kioskHint', label: 'When enabled, launch the shared-device screen and set each employee’s kiosk PIN in People.', type: 'note' },
  ]},
  ...
  /* §6.5 */ { key: 'leaves', title: 'Leave Configuration', icon: '\uD83C\uDF34', fields: [
    { id: 'leavePolicyNote', label: 'Per-type entitlements (monthly, seniority, annual, unlimited) are configured below. These tenant knobs control global leave features that are not per policy.', type: 'note' },
...
export function settingsRender() {
  ...
  SECTIONS.forEach(function (sec, idx) {
    ...
    if (sec.key === 'leaves') {
      html += '<div id="setLeavePolicyManager" class="set-leave-policy-mount"></div>';
    }
    if (sec.key === 'attendance') {
      html +=
        '<div class="set-kiosk-launch">' +
          '<button type="button" class="set-section-save set-kiosk-launch-btn" id="setLaunchKiosk">Launch kiosk</button>' +
          '<div class="set-field-hint">Opens the shared tablet screen. Employees identify with PIN — no personal session is left behind.</div>' +
        '</div>';
    }
  ...
  el.innerHTML = html;
  _bindSectionEvents();
  _mountLeavePolicyManager();
  _bindKioskLaunch();
}

function _mountLeavePolicyManager() {
  if (!_container) return;
  const mount = _container.querySelector('#setLeavePolicyManager');
  if (!mount) return;
  renderLeavePolicyManager(mount, { embed: true });
}

function _bindKioskLaunch() {
  if (!_container) return;
  const btn = _container.querySelector('#setLaunchKiosk');
  if (!btn) return;
  btn.addEventListener('click', async function () {
    const toggle = _container.querySelector('[data-path="attendance.kioskEnabled"]');
    const enabledFromUi = toggle ? !!toggle.checked : !!_getNestedVal(_settingsJson, 'attendance.kioskEnabled');
    if (!enabledFromUi) {
      toast('Turn on Kiosk mode and save Attendance Rules first', 'error');
      return;
    }
    const status = await api.get('/api/kiosk/status');
    if (status && status._error) {
      toast(status.message || 'Could not reach kiosk service', 'error');
      return;
    }
    if (!status || !status.enabled) {
      toast('Kiosk is still off on the server — save Attendance Rules, then try again', 'error');
      return;
    }
    navigateToModule('kiosk');
  });
}
...
async function _saveSection(secKey) {
  ...
  /* Keep local cache in sync so Launch kiosk / other actions see saved values */
  if (body.settings_json && typeof body.settings_json === 'object') {
    Object.keys(body.settings_json).forEach(function (key) {
      const val = body.settings_json[key];
      if (val && typeof val === 'object' && !Array.isArray(val) && _settingsJson[key] && typeof _settingsJson[key] === 'object') {
        _settingsJson[key] = Object.assign({}, _settingsJson[key], val);
      } else {
        _settingsJson[key] = val;
      }
    });
  }
