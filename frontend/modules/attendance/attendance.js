...
export function attRender() {
  ...
  if (!list.length) {
    ...
    const roles = typeof window !== 'undefined' && window.BlokHR && window.BlokHR.userRoles;
    const isAdmin =
      (roles && roles.isAdmin) ||
      (session && (session.is_admin || session.role === 'admin'));
    const emptyCta =
      !hasFilters && isAdmin
        ? '<button type="button" class="grid-empty-cta" id="attAddPeopleBtn">Add employees</button>'
        : '';
    grid.innerHTML =
      '<div class="grid-empty">' +
        '<div class="grid-empty-icon">&#128101;</div>' +
        '<div class="grid-empty-text">' +
          (hasFilters ? 'No matches found' : 'No team members yet') +
        '</div>' +
        '<div class="grid-empty-sub">' +
          (hasFilters
            ...
            : 'Add employees to build your roster — they appear here even before clock-in') +
        '</div>' +
        emptyCta +
      '</div>';
    const cta = grid.querySelector('#attAddPeopleBtn');
    if (cta) {
