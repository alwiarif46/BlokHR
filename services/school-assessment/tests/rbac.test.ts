...
describe('school-assessment P12-04 role guard', () => {
  let app: Express;
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of ASSESSMENT_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/assessment${samplePath(policy.pattern)}`;
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/assessment/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = ASSESSMENT_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/assessment${samplePath(first.pattern)}`
      : '/api/assessment/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
