...
describe('school-academics P12-04 role guard', () => {
  let app: Express;
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of ACADEMICS_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/academics${samplePath(policy.pattern)}`;
      ...
      for (const role of roles) {
        ...
        if (policy.roles.includes(role)) {
          ...
          // Handler memberId / L5 scope body checks may still 403; middleware must allow.
          if (
            role === 'teacher' &&
            (policy.requireMemberBinding || policy.scope === 'teacher_section')
          ) {
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/academics/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = ACADEMICS_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/academics${samplePath(first.pattern)}`
      : '/api/academics/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
