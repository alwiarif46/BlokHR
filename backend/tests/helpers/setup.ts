...
import { createCommercialServices, mountCommercialRouters } from '../../src/commercial/mount';
import {
...
export async function createTestApp(): Promise<{
  ...
  const capturePlatform = await createCapturePlatformBundle(config, testLogger, {
    monolithDb: db,
    entitlements: commercial.entitlements,
  });
  ...
  const app = createApp(config, testLogger, (a) => {
    ...
    const leaveRouter = createLeaveRouter(db, testLogger);
    a.use('/api', clockRouter);
    a.use('/api', leaveRouter);
    const regRouter = createRegularizationRouter(db, testLogger);
    a.use('/api', regRouter);
    const bdMeetingRouter = createBdMeetingRouter(db, testLogger);
