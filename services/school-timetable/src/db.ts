...
export interface SchoolTimetableDb {
...
/**
 * Lightweight sql.js wrapper owned by the school-timetable service.
 */
export class SchoolTimetableSqlite implements SchoolTimetableDb {
  ...
  static async create(dbPath: string): Promise<SchoolTimetableSqlite> {
    const engine = new SchoolTimetableSqlite(dbPath);
...
export async function runSchoolTimetableMigrations(
  db: SchoolTimetableDb,
