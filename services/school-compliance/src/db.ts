...
import { seedGlobalComplianceItems } from './seed-compliance-items';

export interface SchoolComplianceDb {
...
export class SchoolComplianceSqlite implements SchoolComplianceDb {
  ...
  static async create(dbPath: string): Promise<SchoolComplianceSqlite> {
    const engine = new SchoolComplianceSqlite(dbPath);
...
export async function runSchoolComplianceMigrations(
  db: SchoolComplianceDb,
  ...
  await seedGlobalComplianceItems(db);
}
