...
export function createAttendanceTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolAttendanceSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-attendance.db',
    env: opts.env,
    create: (p) => SchoolAttendanceSqlite.create(p),
    migrate: (db) => runSchoolAttendanceMigrations(db, opts.migrationsDir),
  });
}
