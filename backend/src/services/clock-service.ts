...
/** Active employee from the directory roster (for team board). */
...
export interface RosterPort {
  listActiveMembers(): Promise<RosterMember[]>;
}
...
export class ClockService {
  constructor(
    ...
    private readonly roster?: RosterPort,
  ) {}
  ...
  /** Get board data: active roster merged with attendance rows for the date. */
  async getBoard(date: string): Promise<{
    ...
    const enrich = async (r: (typeof records)[number]): Promise<Record<string, unknown>> => {
    ...
    if (this.roster) {
      try {
        roster = await this.roster.listActiveMembers();
      } catch (err) {
        this.logger.warn({ err }, 'Failed to load directory roster for board; falling back to daily rows');
      }
    }
    ...
    for (const m of roster) {
      ...
      seen.add(email);
      const daily = byEmail.get(email);
      if (daily) {
        people.push(await enrich(daily));
      } else {
        people.push({
          email,
          name: m.name,
