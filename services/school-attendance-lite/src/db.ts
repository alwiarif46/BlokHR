...
export class SchoolSqlite implements SchoolDb {
  ...
  static async create(dbPath: string): Promise<SchoolSqlite> {
    const engine = new SchoolSqlite(dbPath);
...
export async function runSchoolMigrations(db: SchoolDb, migrationsDir: string): Promise<void> {
  await db.run(`
