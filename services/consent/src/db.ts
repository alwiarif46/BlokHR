...
export class ConsentSqlite implements ConsentDb {
  ...
  static async create(dbPath: string): Promise<ConsentSqlite> {
    const engine = new ConsentSqlite(dbPath);
...
export async function runConsentMigrations(db: ConsentDb, migrationsDir: string): Promise<void> {
  await db.run(`
