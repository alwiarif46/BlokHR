...
describe('school-transport P12-04 role guard', () => {
  let app: Express;
  ...
  it('table-driven allow/deny', async () => {
    ...
    for (const policy of TRANSPORT_ROUTE_POLICIES) {
      if (policy.internalOnly) continue;
      const url = `/api/transport${samplePath(policy.pattern)}`;
  ...
  it('deny-by-default on unregistered path', async () => {
    const res = await request(app)
      .get('/api/transport/t1/__no_policy__/deep')
      .set(staff('admin'));
  ...
  it('missing internal secret → 401', async () => {
    const first = TRANSPORT_ROUTE_POLICIES.find((p) => !p.internalOnly && p.method === 'GET' && p.roles.length > 0);
    const url = first
      ? `/api/transport${samplePath(first.pattern)}`
      : '/api/transport/t1/no-such-route';
    const res = await request(app)
      .get(url)
      .set({
