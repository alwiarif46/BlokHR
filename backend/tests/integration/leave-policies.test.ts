...
describe('Configurable Leave Rules', () => {
  ...
  describe('POST /api/leave-policies/accrue-now', () => {
    it('rejects non-admin callers', async () => {
      const res = await request(app)
        .post('/api/leave-policies/accrue-now')
        .send({})
    ...
    it('credits pto_balances for active members from policies', async () => {
      ...
      await db.run(
        `INSERT INTO members (id, email, name, group_id, member_type_id, active, joining_date)
         VALUES (?, ?, ?, ?, ?, 1, ?)`,
        ['m-alice', 'alice@shaavir.com', 'Alice', 'g1', 'fte', '2024-01-01'],
      );
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.accrualCount).toBeGreaterThan(0);

      const balance = await db.get<{ accrued: number }>(
      ...
      const leaveBal = await request(app)
        .get('/api/leaves/balances?email=alice@shaavir.com')
        .set('X-User-Email', 'alice@shaavir.com');
      expect(leaveBal.status).toBe(200);
      expect(leaveBal.body.balances.length).toBeGreaterThan(0);
      const casual = leaveBal.body.balances.find(
        (b: { type: string }) => b.type === 'Casual',
      );
      expect(casual).toBeDefined();
      expect(casual.remaining).toBe(1);
    });
  });
});
