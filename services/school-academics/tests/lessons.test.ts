...
describe('school-academics lessons (P3-03)', () => {
  ...
  it('state machine: editable draft/changes_requested; submitted/approved locked', async () => {
    ...
    const patched = await request(app)
      .patch(`/api/academics/t1/lessons/${draft.id}`)
      .send({ title: 'Edited' });
    expect(patched.status).toBe(200);
    expect(patched.body.title).toBe('Edited');

    await request(app).post('/api/academics/t1/lessons/submit-week').send({
      teacher_member_id: 'teacher-1',
      week_start: '2025-09-01',
    });
    ...
    const reviewed = await request(app)
      .post(`/api/academics/t1/lessons/${draft.id}/review`)
      .send({
    ...
    await request(app)
      .post(`/api/academics/t1/lessons/${d2.id}/review`)
      .send({ decision: 'approved', reviewed_by: 'hod-1' });
    const approvedLock = await request(app)
      .patch(`/api/academics/t1/lessons/${d2.id}`)
      .send({ title: 'Nope2' });
  ...
  it('bulk submit emits one week_submitted event', async () => {
    ...
    const res = await request(app).post('/api/academics/t1/lessons/submit-week').send({
      teacher_member_id: 'teacher-1',
      week_start: '2025-09-01',
    });
  ...
  it('deterministic review-sample is stable', async () => {
    ...
    for (const id of ids) {
      await request(app)
        .post(`/api/academics/t1/lessons/${id}/review`)
  ...
  it('stale list and provenance persisted', async () => {
    ...
    const stale = await request(app).get('/api/academics/t1/lessons/stale?days=21');
  ...
  it('tenant isolation for lessons', async () => {
    ...
    const unit2 = await request(app)
      .post(`/api/academics/t2/courses/${course2.body.id}/units`)
      .send({ label: 'U', planned_weeks: 1 });

    const l1 = await createDraft({ title: 'T1 only' });
    await request(app).post('/api/academics/t2/lessons').send({
    ...
    const list1 = await request(app).get('/api/academics/t1/lessons');
    const list2 = await request(app).get('/api/academics/t2/lessons');
