...
export function createFeesTenantPool(opts: {
  legacyPath: string;
  migrationsDir: string;
  env?: NodeJS.ProcessEnv;
}): TenantSqlitePool<SchoolFeesSqlite> {
  return new TenantSqlitePool({
    legacyPath: opts.legacyPath,
    serviceFile: 'school-fees.db',
    env: opts.env,
    create: (p) => SchoolFeesSqlite.create(p),
    migrate: (db) => runSchoolFeesMigrations(db, opts.migrationsDir),
  });
}
