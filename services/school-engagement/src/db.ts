...
import { seedGlobalMessageTemplates } from './seed-message-templates';

export interface SchoolEngagementDb {
...
export class SchoolEngagementSqlite implements SchoolEngagementDb {
  ...
  static async create(dbPath: string): Promise<SchoolEngagementSqlite> {
    const engine = new SchoolEngagementSqlite(dbPath);
...
export async function runSchoolEngagementMigrations(
  db: SchoolEngagementDb,
  ...
  await seedGlobalMessageTemplates(db);
}
