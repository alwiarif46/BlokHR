...
export class CaptureSqlite implements CaptureDb {
  ...
  static async create(dbPath: string): Promise<CaptureSqlite> {
    const engine = new CaptureSqlite(dbPath);
...
export async function runCaptureMigrations(db: CaptureDb, migrationsDir: string): Promise<void> {
  await db.run(`
