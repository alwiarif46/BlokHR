...
export interface SchoolAttendanceDb {
...
/**
 * Lightweight sql.js wrapper owned by the school-attendance domain service.
 */
export class SchoolAttendanceSqlite implements SchoolAttendanceDb {
  ...
  static async create(dbPath: string): Promise<SchoolAttendanceSqlite> {
    const engine = new SchoolAttendanceSqlite(dbPath);
...
export async function runSchoolAttendanceMigrations(
  db: SchoolAttendanceDb,
