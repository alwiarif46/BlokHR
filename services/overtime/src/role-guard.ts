...
export interface RoutePolicy {
  ...
  pattern: RegExp;
  roles: Role[];
  /** L5 tag — reserved for future scope hooks */
  scope?: 'self';
...
export function staffFromHeaders(
  ...
  const tenantId = String(req.headers['x-blok-tenant'] ?? '').trim();
  return { ok: true, email, role, isAdmin, memberId, tenantId };
}
...
export function asRole(
  ...
  if (opts.memberId === null) {
    ...
    h['X-Blok-Member'] = opts.memberId ?? `member-${role}`;
  }
  return h;
}
